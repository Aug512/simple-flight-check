...
export const getQuotes = (responseQuotes, rawResponse, monthsLib, state) => {   //получаем данные с API и приводим в удобный формат (без лишних свойств, PascalCase -> camelCase)
  return responseQuotes.map( quote => {
    const id = generateId(quote.OutboundLeg.DepartureDate, quote.QuoteDateTime) + quote.MinPrice
    return {
      id,
